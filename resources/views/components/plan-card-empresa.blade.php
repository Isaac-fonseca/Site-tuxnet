...
<div class="col-xl-3 col-lg-4 col-md-6 col-sm-10 col-11 mx-auto mb-4 d-flex align-items-stretch">
    <div class="card plano-card plano-empresa-card {{ $plano['destaque'] ?? false ? 'border-secondary-tuxnet shadow-lg' : 'shadow-sm' }} h-100">
        @if($plano['destaque'] ?? false)
            <div class="plano-destaque-badge">RECOMENDADO PME</div>
        @endif
        <div class="card-header text-center bg-primary-tuxnet text-white py-3">
            <h3 class="h4 mb-0 font-montserrat">{{ $plano['nome_plano_card'] ?? ($plano['velocidade'] ?? 'Plano Empresarial') }}</h3>
        </div>
        ...
        <div class="card-footer text-center bg-transparent border-top-0 pb-3 pt-0">
            <a href="{{ $plano['link_contratar'] ?? '#' }}" class="btn {{ $plano['destaque'] ?? false ? 'btn-secondary-tuxnet' : 'btn-primary-tuxnet' }} btn-lg w-100 contratar-btn">Solicitar Orçamento</a>
        </div>
    </div>
</div>
